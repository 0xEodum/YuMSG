...
class ChatManager {
  ...
  /// Миграция старых чатов на новый формат
  Future<void> migrateChats() async {
    debugPrint('Starting chat migration...');
    ...
    try {
      ...
      for (final chat in chats) {
        ...
        final newChatId = generateChatId(participantId);
        
        // Если ID уже в новом формате, пропускаем
        if (oldChatId == newChatId) {
          continue;
        }
        
        debugPrint('Migrating chat $oldChatId to $newChatId');
        
        // Получаем сообщения и ключи
        final messages = await _chatRepository.getMessages(oldChatId);
        final keys = await _chatRepository.getChatKeys(oldChatId);
        
        // Создаем новый чат с тем же содержимым, но новым ID
        final newChat = chat.copyWith(id: newChatId);
        await _chatRepository.saveChat(newChat);
        
        // Мигрируем сообщения
        for (final message in messages) {
          final newMessage = message.copyWith(chatId: newChatId);
          await _chatRepository.saveMessage(newMessage);
        }
        
        // Мигрируем ключи, если есть
        if (keys != null) {
          final newKeys = keys.copyWith(chatId: newChatId);
          await _chatRepository.saveChatKeys(newKeys);
        }
        
        // Удаляем старый чат
        await _chatRepository.deleteChat(oldChatId);
        
        debugPrint('Migration completed for chat with user $participantId');
      }
      
      debugPrint('Chat migration completed successfully');
    } catch (e) {
      debugPrint('Error during chat migration: $e');
      debugPrintStack();
    }
  }
}